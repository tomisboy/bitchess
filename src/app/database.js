...
module.exports = {
  ...
  /////////ELO
  updategames: function (playerwon) {
    try {
      var con = mysql.createConnection(dbconfig);
      con.connect()
      con.query("UPDATE users SET gameswon = gameswon + 1 WHERE id = ?", [playerwon], function (err, result) {
        if (err) throw err;
  ...
  updateRating: function (id, rating) {
    try {
      var con = mysql.createConnection(dbconfig);
      con.connect()
      con.query("UPDATE users SET rating = ? WHERE id = ?", [rating, id], function (err, result) {
        if (err) throw err;
      }),
        con.end
    } catch (e) {
